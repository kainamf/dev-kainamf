import { Code2, ArrowRight } from 'lucide-react';
import React, { useState } from 'react';

interface HeroSectionProps {
  theme: 'dark' | 'light';
  themeClasses: { text: string; card: string };
  scrollY: number;
  isHovered: boolean;
  setIsHovered: (v: boolean) => void;
  technologies: string[];
  currentTech: number;
  scrollToSection: (id: string) => void;
}

const HeroSection: React.FC<HeroSectionProps> = ({ theme, themeClasses, scrollY, isHovered, setIsHovered, technologies, currentTech, scrollToSection }) => {
  const [clickCount, setClickCount] = useState(0);
  const [rotation, setRotation] = useState(0);
  const [exploded, setExploded] = useState(false);
  const [miniIcons, setMiniIcons] = useState<{id:number, x:number, y:number, blur:number}[]>([]);

  const handleIconClick = () => {
    if (exploded) return;
    const newCount = clickCount + 1;
    setClickCount(newCount);
    setRotation((prev) => prev + 30);
    if (newCount >= 5) {
      setExploded(true);
      // Cria 200 mini-ícones com ângulos, distâncias e blur aleatórios cobrindo toda a tela
      setMiniIcons(Array.from({length: 100}, (_, i) => {
        const angle = Math.random() * 2 * Math.PI; // radianos
        const distance = 600 + Math.random() * 500;
        const blur = 8 + Math.random() * 16;
        // Calcula X e Y finais
        const x = Math.cos(angle) * distance;
        const y = Math.sin(angle) * distance;
        return {id: i, x, y, blur};
      }));
      setTimeout(() => {
        setExploded(false);
        setClickCount(0);
        setRotation(0);
        setMiniIcons([]);
      }, 3400);
    }
  };

  return (
    <section id="home" className="min-h-screen flex items-center justify-center p-4 pt-24 sm:pt-32">
      <div className="w-full max-w-6xl text-center flex flex-col items-center gap-8">
        <div className="relative mb-8">
          {/* Ícone principal some durante a explosão */}
          <div
            className={`inline-flex items-center justify-center w-24 h-24 bg-gradient-to-br from-[#7CDA3D] to-[#7CDA3D]/80 rounded-3xl transition-all duration-300 hover:scale-110 hover:rotate-12 shadow-2xl shadow-[#7CDA3D]/20 cursor-pointer ${exploded ? 'opacity-0 scale-75' : 'opacity-100'}`}
            style={{ transform: `rotate(${rotation}deg)` }}
            onClick={handleIconClick}
          >
            <Code2 size={48} className="text-[#030303] animate-pulse" />
          </div>
          {/* Mini-ícones explodindo */}
          {exploded && miniIcons.map(({id, x, y, blur}) => (
            <span
              key={id}
              className="mini-icon"
              style={{
                position: 'absolute',
                left: '50%',
                top: '50%',
                filter: `blur(${blur}px)` ,
                transform: `translate(-50%, -50%) scale(1)`,
                pointerEvents: 'none',
                animation: `explode-mini-icon-cartesian 3.2s cubic-bezier(.5,-0.5,.5,1.5) forwards`,
                animationDelay: `${id * 0.002}s`,
                '--explode-x': `${x}px`,
                '--explode-y': `${y}px`
              } as React.CSSProperties}
            >
              <Code2 size={36} className="text-[#7CDA3D] drop-shadow-2xl opacity-80" />
            </span>
          ))}
        </div>
        <h1 className={`text-4xl xs:text-5xl sm:text-6xl md:text-8xl font-bold ${themeClasses.text} mb-8 relative leading-tight break-words`}>
          <span className="block">Desenvolvedor</span>
          <span className="block text-[#7CDA3D] bg-gradient-to-r from-[#7CDA3D] to-[#7CDA3D]/70 bg-clip-text text-transparent animate-pulse">
            Full Stack
          </span>
        </h1>
        <p className={`text-xl md:text-2xl ${theme === 'light' ? 'text-gray-600' : 'text-gray-300'} max-w-3xl mx-auto leading-relaxed mb-8 opacity-0 animate-fade-in-up`} style={{animationDelay: '1s', animationFillMode: 'forwards'}}>
          Criação de sites e landing pages modernas, responsivas e otimizadas para conversão
        </p>
    

        <div className="flex flex-col sm:flex-row gap-8 justify-center">
          <button 
            onClick={() => scrollToSection('contact')}
            className="group/btn bg-[#7CDA3D] text-[#030303] px-10 py-4 rounded-2xl font-bold text-lg hover:bg-[#7CDA3D]/90 transition-all duration-300 flex items-center justify-center gap-3 hover:scale-105 hover:shadow-2xl hover:shadow-[#7CDA3D]/30 relative overflow-hidden"
          >
            <span className="relative z-10">Iniciar Projeto</span>
            <ArrowRight size={24} className="group-hover/btn:translate-x-2 transition-transform duration-300" />
            <div className="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover/btn:translate-x-[100%] transition-transform duration-700 skew-x-12" />
          </button>
          <button 
            onClick={() => scrollToSection('projects')}
            className="border-2 border-[#7CDA3D] text-[#7CDA3D] px-10 py-4 rounded-2xl font-bold text-lg hover:bg-[#7CDA3D]/10 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-[#7CDA3D]/20 relative overflow-hidden group/btn2"
          >
            <span className="relative z-10">Ver Portfolio</span>
            <div className="absolute inset-0 bg-[#7CDA3D]/5 scale-x-0 group-hover/btn2:scale-x-100 transition-transform duration-300 origin-left" />
          </button>
        </div>
      </div>
    </section>
  );
};

export default HeroSection;
